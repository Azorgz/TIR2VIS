...
class ComboGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.segMask_Fus_update = self.Tensor(opt.batchSize, opt.num_class, opt.fineSize, opt.fineSize).long()
        ...
        if self.isTrain:
            ...
            self.criterionBC_VIS = lambda y, t, r, v, f, m: BiasCorrLossV2(y, t, r, v, f, m)
...
class GanColorCombo(ComboGANModel):
    ...
    def __init__(self, opt):
        ...
        self.criterionSaturation = lambda x: torch.sum(nn.ReLU()(x * 0.5 + 0.5).mean(dim=1) - 0.95) / (
                ((x * 0.5 + 0.5).mean(dim=1) - 0.95 > 0).sum() + 1e-6)
    ...
    def backward_G(self):
        ...
        encoded_BC, segMask_Fus, self.rec_real_C = self.netG.fusion_features(encoded_B, encoded_C,
                                                                             self.mask, self.real_B, self.real_C,
                                                                             p_color=self.pedestrian_color,
        ...
        # D_A(G_A(B))
        self.fake_A = self.netG.decode(encoded_B, self.DA)
        self.loss_G[1] += self.criterionGAN(self.pred_real_A, self.netD.forward(self.fake_A, self.DA), False) * 2 \
        ...
        self.loss_G[2] += self.criterionSSIM((self.fake_A_C + 1) / 2, (self.real_B + 1) / 2) * self.lambda_ssim
        # D_A(G_A(Fus))
        self.fake_A_BC = self.netG.decode(encoded_BC, self.DA)
        self.loss_G[2] += self.criterionGAN(self.pred_real_A, self.netD.forward(self.fake_A_BC, self.DA), False) * 2 \
        ...
        self.loss_G[1] += self.criterionGAN(self.pred_real_B, self.netD.forward(self.fake_B_C, self.DB), False) \
            if self.cond('EA', 'DB') else self.null
        # D_C(G_C(A))
        ...
        if self.cond('Fus'):
            ...
            self.loss_cycle[self.DC] += loss_cycle(self.rec_C_A_BC*self.mask, self.real_C*self.mask)*3 \
        ...
        if self.epoch > 75:
            rec_encoded_BC_A, _, _ = self.netG.fusion_features(rec_encoded_A, rec_encoded_A_C, None,
                                                               self.fake_A,
                                                               self.fake_A_C, p_color=self.pedestrian_color,
                                                               detach_seg=self.epoch < 40)
        else:
            rec_encoded_BC_A, segMask_Fus, _ = self.netG.fusion_features(rec_encoded_A, rec_encoded_A_C, None,
                                                                         self.fake_A,
                                                                         self.fake_A_C, p_color=self.pedestrian_color,
                                                                         detach_seg=self.epoch < 40)
        ...
        # "Random size for segmentation network training. Then, retain original image size."
        if self.netS_freezing_idx == 0.0:
            rand_scale = torch.randint(32, 60 if 'laptop' in socket.gethostname() else 80, (1, 1))  # 32, 80
            rand_size = int(rand_scale.item() * 4)
            rand_size_B = int(rand_scale.item() * 4)
        else:
        ...
        if self.epoch >= 20:  # epoch 20-30
            ...
            if self.epoch >= 30:  # epoch 30-100
                ...
                fake_A_C_pred_d, _ = self.netS.forward(fake_A_C_s.detach(), self.DA)
        ...
        if self.DB_GT_update_idx == 0.0:  # epoch 0-40
            ...
            if self.netS_freezing_idx < 1:
                ...
                self.loss_S_rec[self.DB] += self.lambda_sc * seg_loss(fake_B_pred, self.SegMask_A_update.long())
                self.loss_S_rec[self.DC] += self.lambda_sc * seg_loss(fake_C_A_pred, self.SegMask_A_update.long())
                ...
                if self.cond('C', dom='S'):
                    self.loss_S_enc[self.DC] = self.lambda_sc * seg_loss(real_C_pred, SegMask_B_update2[0].long())
        ...
        if self.lambda_acl > 0:  # epoch > 40
            ...
            Com_RealVis = out_FG_RealVis + out_FG_RealVis_flip
            ###Traffic Light Luminance Loss
            loss_tll = self.criterionTLL(self.fake_A_BC, real_B_Mask, self.real_B.detach(), self.gpu_ids[0]) * 3
            ...
            loss_TLight_color = self.criterionTLC(self.real_B, self.fake_A_BC, real_B_Mask,
                                                  Com_RealVis, ComIR_Light_Mask, HL_Mask, self.gpu_ids[0]) * 3
            ...
            if self.Vis_com is not None:
                encoded_Vis_com = self.netG.encode(self.Vis_com, self.DA)
                self.fake_B_Vis_com = self.netG.decode(encoded_Vis_com, self.DB)
                self.fake_C_Vis_com = self.netG.decode(encoded_Vis_com, self.DC)
                if torch.sum(FakeVis_FG_Mask) > 0.0:
                    loss_ACL_A = (self.criterionPixCon(self.fake_B_Vis_com, self.real_B, FakeVis_FG_Mask,
                                                       self.opt.ssim_winsize) +
                                  self.criterionPixCon(self.fake_C_Vis_com, self.real_C, FakeVis_FG_Mask,
                                                       self.opt.ssim_winsize))
                else:
                    loss_ACL_A = 0.0
                ...
                if torch.sum(FakeVis_FG_Mask_flip) > 0.0:
                    loss_ACL_A_flip = (self.criterionPixCon(self.fake_B_Vis_com, torch.flip(self.real_B, dims=[3]),
                                                            FakeVis_FG_Mask_flip, self.opt.ssim_winsize) +
                                       self.criterionPixCon(self.fake_C_Vis_com, torch.flip(self.real_C, dims=[3]),
                                                            FakeVis_FG_Mask_flip, self.opt.ssim_winsize))
                else:
                    loss_ACL_A_flip = 0.0
                self.loss_AC[self.DA] = loss_ACL_A + loss_ACL_A_flip
        ...
        self.loss_sga[self.DC] += (self.lambda_sga * self.criterionSGAIR(combined_grad.detach(),
                                                                         ...
                                                                         self.patch_num_sqrt, self.grad_th_IR) +
                                   self.lambda_ssim * self.criterionBC_VIS(self.SegMask_A.detach(), self.fake_C_A,
                                                                           self.real_A.detach(), self.rec_A_C,
                                                                           self.EdgeMap_A,
                                                                           self.gpu_ids[0])) if self.cond('EC', 'Fus',
                                                                                                          'DA',
                                                                                                          'EB') else self.null
        ...
        if self.epoch > 75:
            self.loss_sga[self.DC] += (self.lambda_ssim *
                                       self.criterionVISClsDis(self.SegMask_B_update[0].detach(), self.fake_A_BC,
                                                               self.rec_real_C.detach(), self.gpu_ids[0]) +
                                       self.lambda_ssim *
                                       self.criterionVISClsDis(self.SegMask_B_update[0].detach(), self.rec_C_A_BC,
                                                               self.rec_real_C.detach(), self.gpu_ids[0]))
        ...
        if self.lambda_enc > 0:
            ...
            loss_enc_C += self.criterionLatent(rec_encoded_A_C, rec_encoded_A) if self.cond('EC', 'EB',
                                                                                            'DA') else self.null
        ...
        if self.lambda_fwd > 0:
            ...
            loss_fwd_C = self.criterionIdt(self.fake_A_C, self.real_A)
        ...
        if not testing:
            if self.opt.simple_train:
                self.visuals = {'real_A': self.real_A, 'real_B': self.real_B,
                                ...
                                'seg_mask_Fus_up': self.segMask_Fus_update}
            else:
