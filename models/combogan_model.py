...
class ComboGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.fake_A = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.fake_B = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.fake_BC = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.fake_A_BC = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.fake_C_A = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.fake_C_B = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.fake_A_C = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.fake_B_C = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)

        self.rec_A = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.rec_B_A_BC = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.rec_C_A_BC = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.rec_C_B = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.rec_C_A = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.rec_A_C = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.rec_B_C = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        self.rec_B_BC = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize)
        self.rec_C_BC = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
        ...
        if self.isTrain:
            ...
            self.criterionGAN = lambda r, f, v: (GANLoss(r[0], f[0], v) + \
                                                 GANLoss(r[1], f[1], v) + \
                                                 GANLoss(r[2], f[2], v)) / 3
...
class GanColorCombo(ComboGANModel):
    ...
    def __init__(self, opt):
        ...
        self.criterionColor = lambda x, y, z: HistogramLoss(x, y, z)
        self.partial_train_net = opt.partial_train if opt.partial_train is not None else \
    ...
    def backward_G(self):
        ...
        self.loss_color = self.null #self.criterionColor(self.rec_A_C, self.real_A, self.SegMask_A) * self.lambda_color \
            # if self.cond('EC', 'DA', 'EA', 'DC') else self.null
        ...
        if self.lambda_enc > 0:
            loss_enc_A = self.criterionLatent(rec_encoded_A, encoded_A)
            loss_enc_B = self.criterionLatent(rec_encoded_B, encoded_B)
            loss_enc = loss_enc_A + loss_enc_B
        else:
            loss_enc = 0
        ...
        if self.lambda_fwd > 0:
            loss_fwd_A = self.criterionIdt(self.fake_B, self.real_A)
            loss_fwd_B = self.criterionIdt(self.fake_A, self.real_B)
            loss_fwd_C_A = (self.criterionIdt(self.fake_C_A, self.real_A) +
                            self.criterionIdt(self.fake_A_C * self.mask, self.real_C * self.mask))
            loss_fwd = loss_fwd_A + loss_fwd_B + loss_fwd_C_A
        else:
            loss_fwd = 0
    ...
    #     encoded_A = self.netG.encode(self.real_A, self.DA)
    #     encoded_B = self.netG.encode(self.real_B, self.DB)
    #     # self.fake_BC = self.netG.fusion_features(self.real_B, self.real_C, self.epoch)
    #
    #     # Optional identity "autoencode" loss ###############################
    #     if self.lambda_idt > 0:
    #         # Same encoder and decoder should recreate image
    ...
    #         loss_idt_A = self.criterionIdt(idt_A, self.real_A)
    #         idt_B = self.netG.decode(encoded_B, self.DB)
    #         loss_idt_B = self.criterionIdt(idt_B, self.fake_BC)
    #     else:
    #         loss_idt_A, loss_idt_B = 0, 0
    #
    #     # GAN loss
    ...
    #                                     mode='bilinear', align_corners=False)
    #         ##########Fake_IR_Composition, OAMix-TIR
    #         self.SegMask_B_update = F.interpolate(self.SegMask_B_update.float().unsqueeze(0), size=[256, 256], mode='nearest')
    #         FakeIR_FG_Mask, out_FG_FakeIR, out_FG_RealVis, FakeIR_FG_Mask_flip, out_FG_FakeIR_flip, out_FG_RealVis_flip, FakeIR_FG_Mask_ori, HL_Mask, ComIR_Light_Mask = \
    #             self.get_FG_MergeMask(self.SegMask_A.detach(), fake_A_Mask, self.real_A, self.fake_B.detach(),
    #                                   self.gpu_ids[0])
    #         self.IR_com = self.get_IR_Com(FakeIR_FG_Mask, FakeIR_FG_Mask_flip, out_FG_FakeIR, out_FG_FakeIR_flip,
    ...
    #     if self.netS_freezing_idx == 1.0:
    #         ###Conditional Gradient Repair loss
    #         loss_cgr = self.criterionCGR(self.fake_A, self.SegMask_B_update.unsqueeze(0).detach(), self.real_B.detach(),
    #                                      self.gpu_ids[0])
    #         ########Domain-specific losses include CGR loss and ACA loss.
    #         self.loss_DS[self.DB] = self.lambda_CGR * loss_cgr
    #         self.loss_DS[self.DA] = self.criterionACA(self.SegMask_A_update.unsqueeze(0).detach(), encoded_A.detach(), \
    #                                                   self.SegMask_B_update.unsqueeze(0).detach(), rec_encoded_BC, 4, 100000,
    #                                                   self.gpu_ids[0])
    #     else:
    #         self.loss_DS[self.DA] = 0.0
    #         self.loss_DS[self.DB] = 0.0
    ...
    #     ####The last three terms of loss_sga[self.DA] denote the monochromatic regularization term, the temperature
    #     # regularization term, and the bias correction loss, respectively.
    #     self.loss_sga[self.DA] = self.lambda_sga * self.criterionSGAVis(self.EdgeMap_A, self.get_gradmag(self.fake_BC),
    #                                                                     self.patch_num_sqrt, self.grad_th_vis) + \
    #                              torch.max(torch.max(self.fake_BC, 1)[0] - torch.min(self.fake_BC, 1)[0]) + \
    #                              self.lambda_ssim * self.criterionIRClsDis(self.SegMask_A.detach(), self.fake_BC,
    #                                                                        self.real_A.detach(), self.gpu_ids[0]) + \
    #                              self.lambda_ssim * self.criterionBC(self.SegMask_A.detach(), self.fake_BC,
    #                                                                  self.real_A.detach(), self.rec_A, self.EdgeMap_A,
    #                                                                  self.gpu_ids[0])
    #     self.loss_sga[self.DB] = self.lambda_sga * self.criterionSGAIR(self.EdgeMap_B, self.get_gradmag(self.fake_A),
    #                                                                    self.patch_num_sqrt, self.grad_th_IR)
    ...
    #     # Optional loss on downsampled image before and after
    #     if self.lambda_fwd > 0:
    #         loss_fwd_A = self.criterionIdt(self.fake_BC, self.real_A)
    #         loss_fwd_B = self.criterionIdt(self.fake_A, self.fake_BC)
    #     else:
    #         loss_fwd_A, loss_fwd_B = 0, 0
    #
    #     # combined loss
