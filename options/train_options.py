...
class TrainOptions(BaseOptions):
    def initialize(self):
        ...
        self.parser.add_argument('--which_epoch', type=int, default=75,
                                 help='which epoch to load if continuing training')
        self.parser.add_argument('--epoch_load', type=validate_epoch_load, default='latest', #validate_epoch_load({'G': 'latest', 'G6': -1, 'S0': 100, 'S1': 100, 'D': 'latest', 'S': 'latest'}),  #'latest', #validate_epoch_load(
        ...
        self.parser.add_argument("--partial_train", type=Union[dict, None],
                                 default=None, #{'G': [0, 1, 4, 5, 6], 'D': [0, 1, 2], 'S': [1, 2]},
